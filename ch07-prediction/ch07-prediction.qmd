...
Features will be computed cumulatively for all course weeks up to the current one (i.e., when computing features in week k, data for weeks 1..k will be used). Functions for computing feature values and creation of a feature set are in the 'feature_creation.R' script.
...
```
According to all measures, model #2, with the data from the first two weeks of the course, is the best one. So, the students' interaction with the course activities in the first two weeks is the most predictive of their overall course outcome.    

Examine the importance of features in the best model 
```{r}
compute_and_plot_variable_importance(models[[2]])
```
Entropy-based features, that is, features reflective of the regularity of study are those with the highest predictive power, which is in line with the literature.
...
To determine if linear modelling will be applicable, we'll make use of the data from the 1st week of the course and try to build a linear regression model 
```{r message=FALSE}
...
As a side note, in addition to checking all assumptions at once, using the `check_model` function, one can also check each assumption individually using appropriate functions from the the *performance* R package. For example, from the figure above, we can not clearly see the X-axis of the collinearity plot, so,if we want to examine collinearity more closely, we can invoke the function `check_collinearity`:
```{r}
check_collinearity(lr)
```
From the function's result, we can now clearly see the VIF (Variance Inflation Factor) values for all the features and a confirmation that the assumption of the absence of multi-colinearity is satisfied. 

The whole list of functions for different ways of checking regression models is available at: https://easystats.github.io/performance/reference/index.html
...
```
Again, the model with data from the first two weeks is the best in terms of the error it makes (RMSE, MAE).
So, as early as the 2nd week of the course, we can make a prediction of the final grade and expect to predict it with ~0.5 error in grade points. Considering the range for the final grade (0-10), this is a small error.  

Examine the importance of features in the best model 
```{r}
compute_and_plot_variable_importance(regression_models[[2]])
```
Again, regularity of study features are among the most important ones